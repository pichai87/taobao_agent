...
public class RunService implements RunUseCases {
    ...
    public Run submit(String owner,String key,Request q) {
        ...
        QuestionConstraints.validate(q);
        // 日期字段是执行依据；不让模型凭空补日期。
    ...
    private void dispatch(Run run) {
        ...
        try {
            executor.execute(()-> {
                try {
                    ...
                    store.failWithEvent(run.id(),code);
        ...
        } catch(RejectedExecutionException e) {store.failWithEvent(run.id(),"CAPACITY_EXCEEDED");}
    }
    ...
    public Run cancel(String owner,String id) {
        ...
        if(!Set.of(Status.WAITING_FOR_REVIEW,Status.QUEUED,Status.RUNNING,Status.INTERRUPTED).contains(run.status())
            || !store.cancelWithEvent(id,run.status())) throw new BusinessException("INVALID_RUN_STATE");
        return get(owner,id);
    }
}
